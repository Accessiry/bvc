...
precision_file = config.SAFE_RESULT_DIR + os.sep + PREFIX + "_precision.csv"
AUC_file = config.SAFE_RESULT_DIR + os.sep + PREFIX + "_AUC.csv"
recall_file = config.SAFE_RESULT_DIR + os.sep + PREFIX + "_recall.csv"
f1score_file = config.SAFE_RESULT_DIR + os.sep + PREFIX + "_f1score.csv"
accuracy_file = config.SAFE_RESULT_DIR + os.sep + PREFIX + "_accuarcy.csv"


train_label_predict_file = config.SAFE_RESULT_DIR + os.sep + PREFIX + "_train_label_predict_r.csv"
test_label_predict_file = config.SAFE_RESULT_DIR + os.sep + PREFIX + "_test_label_predict_r.csv"
...
def get_batch(label, cfg_str1, cfg_str2, dfg_str1, dfg_str2, fea_str1, fea_str2, num1, num2, max_num, blockembed1,
    ...
    for i in range(B):
        ...
        for node in range(int(num1[i])):       # self connect
            cfg_ori1[node][node] = 1.
        if num1[i] < MAX_B:     # need to patch
            cfg_zero1 = np.zeros([int(num1[i]), (int(MAX_B) - int(num1[i]))])
            cfg_zero2 = np.zeros([(int(MAX_B) - int(num1[i])), int(MAX_B)])
            ...
            cfg_1.append(cut_cfg(cfg_ori1, MAX_B).tolist())
        ...
        if num2[i] < MAX_B:     # need to patch
            cfg_zero1 = np.zeros([int(num2[i]), (int(MAX_B) - int(num2[i]))])
            cfg_zero2 = np.zeros([(int(MAX_B) - int(num2[i])), int(MAX_B)])
            cfg_vec1 = np.concatenate([cfg_ori2, cfg_zero1], axis=1)
            cfg_vec2 = np.concatenate([cfg_vec1, cfg_zero2], axis=0)
            cfg_2.append(cfg_vec2.tolist())
        else:     # need to cut
            cfg_2.append(cut_cfg(cfg_ori2, MAX_B).tolist())
    ...
    w2v_2 = []
    max_instnum1 = 0
    for blk in instnum1:         # 统计batch里基本块中指令个数的最大值，用于w2v补长度
    ...
    for i in range(B):
        ...
        if num1[i] < MAX_B:
            block_zero1 = np.zeros([int(MAX_B - num1[i]), int(max_instnum1), IE])
            w2v_func_vec1 = np.concatenate([inst_func1,block_zero1])
        else:
            w2v_func_vec1 = inst_func1


        w2v_1.append(cut_w2v(w2v_func_vec1,MAX_B,MAX_I).astype(np.float32))
        ...
        if num1[i] < MAX_B:
            bert_zero1 = np.zeros([int(MAX_B - block_embed_vec1.shape[0]),BE])
            # fea_vec1 = np.concatenate([fea_block_1, fea_zero1], axis=0)
            bert_vec1 = np.concatenate([block_embed_vec1, bert_zero1], axis=0)
            bert_1.append(cut_bert(bert_vec1,MAX_B))
        else:
            bert_1.append(cut_bert(block_embed_vec1,MAX_B))
        ...
        if num2[i] < MAX_B:
            block_zero2 = np.zeros([int(MAX_B - num2[i]), int(max_instnum2), IE])
            w2v_func_vec2 = np.concatenate([inst_func2,block_zero2])
            w2v_2.append(cut_w2v(w2v_func_vec2,MAX_B,MAX_I).astype(np.float32))
        else:
            w2v_2.append(cut_w2v(inst_func2,MAX_B,MAX_I).astype(np.float32))
        ...
        if num2[i] < MAX_B:
            bert_zero2 = np.zeros([int(MAX_B - block_embed_vec2.shape[0]), BE])
            bert_vec2 = np.concatenate([block_embed_vec2, bert_zero2], axis=0)
            bert_2.append(cut_bert(bert_vec2,MAX_B))
        else:
            bert_2.append(cut_bert(block_embed_vec2,MAX_B))


    # for i in range (B):
    #     print(len(cfg_1[i]), len(cfg_2[i]), bert_1[i].shape[0], bert_2[i].shape[0],  w2v_1[i].shape[0], w2v_2[i].shape[0])
    # print('123')
    return y, cfg_1, cfg_2, bert_1, bert_2, v_num_1, v_num_2, w2v_1, w2v_2, max_instnum1, max_instnum2#max_num is the same,which is 586
...
#                      batch_size=B, capacity=10)alid_block_embedding2,
#                       list_valid_inst_embedding1, list_valid_inst_embedding2, list_valid_inst_num1,
#                       list_valid_inst_num2],
#                      batch_size=B, capacity=10)
...
with tf.Session(config=tfconfig) as sess:
    ...
    while iter < max_iter:
        ...
        for i in range(total_batch):  # 原数据：total_batch
            ...
            y, cdfg_1, cdfg_2, fea_1, fea_2, v_num_1, v_num_2, w2v_1, w2v_2,inst_max1,inst_max2 \
        ...
        if iter % snapshot == 0:
            ...
            if iter == max_iter:
                fpr_file = config.SAFE_RESULT_DIR + os.sep + PREFIX + '_' + str(iter) + '_fpr.csv'
                tpr_file = config.SAFE_RESULT_DIR + os.sep + PREFIX + '_' + str(iter) + '_tpr.csv'
            total_labels = []
            total_predicts = []
            avg_loss = 0.
            avg_acc = 0.
