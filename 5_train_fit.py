...
def bi_rnn(X_inst_embed,max_inst_num,inst_dim):
    ...
    forward_cell = tf.contrib.rnn.BasicLSTMCell(num_units = inst_dim)
    backward_cell = tf.contrib.rnn.BasicLSTMCell(num_units = inst_dim)
...
def get_batch(label, cfg_str1, cfg_str2, dfg_str1, dfg_str2, fea_str1, fea_str2, num1, num2, max_num, blockembed1,
    ...
    max_instnum1 = 0
    for blk in instnum1:
        for inst in np.array(blk.split(',')).astype(np.int32):
    ...
    for i in range(B):
        ...
        block_embed_arr1 = np.array(blockembed1[i].split(','))
        ...
        bert_zero1 = np.zeros([int(max_num[i] - num1[i]),BE])
